...
class Son extends Father { private int sInt; private long sonL; }
```
...
```
  ...
  - 通过参数 `-XX:-UseCompressedOops` 关闭指针压缩：
...
如果包含引用、数组类型，则保存的都是引用，如下示例（默认开启指针压缩）：

```java
class Father { private int fInt; }
class Son extends Father {private long sL;  private Long sLong; private Father f; private Father[] fArray;}
```

- `ClassLayout.parseInstance(new Son()).toPrintable()` 将打印：
...
- `ClassLayout.parseInstance(new Son[]{new Son()}).toPrintable()` 将打印：

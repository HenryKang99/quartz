...
若 `驱动表 t1 join 被驱动表 t2 on 条件` 且 t1 、t2 行数分别为 M、N，则：

| JOIN 算法                                      | 核心思想                    | 时间复杂度        | 空间复杂度         | 关键前提                       |
| :------------------------------------------- | :---------------------- | :----------- | :------------ | :------------------------- |
| 嵌套循环连接 (Nested-Loop Join, NLJ)               | 用驱动表的行去匹配被驱动表           | O(M * log N) | O(1)          | 被驱动表 (`t2`) 的连接字段上**有索引**  |
| 朴素嵌套循环连接 (Brute-force Nested-Loop Join, BNL) | 两层 for 循环，逐行比较          | O(M * N)     | O(1)          | 被驱动表 (`t2`) 的连接字段上**没有索引** |
| 块嵌套循环连接 (Block Nested Loop, BNL)             | 一次加载一块驱动表数据到内存，再与被驱动表比较 | O(M * N / B) | O(B)          | 无索引，但有 Join Buffer 可用      |
| 哈希连接 (Hash Join)                             | 构建哈希表，再进行探测             | O(M + N)     | O (M) 或 O (N) | MySQL 8.0+，适用于**等值连接**     |

**结论**：被驱动表有索引才使用，并且小表 join 大表，没索引最好不使用。

**分析：**

1. 有索引时，使用 NLJ，复杂度为 `O(M * log N)`，可知 M 影响较大，所以应当使用小表 join 大表；
2. 无索引时，使用 BNL，除了朴素的双层 for 循环，MySQL 使用 join buffer 优化，**减少对被驱动表的扫描次数**，步骤如下：  
    先将驱动表 t1 的数据批量读入 join buffer 中；  
    然后全表扫描被驱动表 t2，对于 t2 的每一行，都与 join buffer 中的数据进行比较；  
    设 join buffer 大小为 B，每批可容纳 K 条 t1 的数据，因此时间复杂度为 `O(M/K * N)`，由于 K 与 B 成正比，可约等于 `O(M*N/B)`；
...
#### join 中 条件写在 on 和 where 后的区别

[参考](https://blog.csdn.net/weixin_43888806/article/details/100057911)

取决于后面的条件，用 `a join b` 举例，条件分为三种：a 表筛选条件、b 表筛选条件、join 关联条件。

筛选条件不管写在 on 或 where 后面，都会先执行，区别是 where 最终会过滤掉不满足条件的数据，而 on 不会（inner join 中 on 等于 where），外连接不满足 on 筛选条件的数据会 union 在结果集后面，即就是外连接的悬浮项；

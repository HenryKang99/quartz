...
- 在 B 树中，中间节点也包含数据，而在 B+ 树中，只有叶子节点包含数据，所以 B 树的查询不够稳定。
- B+ 树叶子节点用指针相连接形成一个链表（应该是一个双向循环链表，图中没有给出），非常适合范围查询，而 B- 树就需要中序遍历才可以。顺带一提为什么 InnoDB 不使用哈希索引，因为哈希索引只适合等值查询，范围查询就得全表扫描。
- InnoDB 中一个索引页就对应一个树节点，默认 16KB，每个索引项中包含搜索码 (大小由主键类型决定) 和指向子树的指针 (6B)；

> [!question] 一个三层的 B+ 树大约可以索引到 1600 万记录  
> 假设一条记录 1KB，一个索引页 16KB，且主键类型为 Bigint，那么：  
> 每个索引项大小 = 搜索码（Bigint, 8B）+ 指针（InnoDB 中为 6B，这里按 8B 计算）≈ 16B；  
> 每个非叶子节点的索引项数 = 索引页大小 / 每个索引项大小，即 `16KB/16B≈1000`；  
> 所以每个非叶子节点索引页可以索引 1 000 个下级索引页，三层 B+ 树可以索引 `1000*1000*(16K/1K)≈1600 万`。
...
主键索引当然是唯一索引，所以这里讨论的普通索引和唯一索引都是 **针对辅助索引** 而言的，需要从查询和更新两个角度考虑。
